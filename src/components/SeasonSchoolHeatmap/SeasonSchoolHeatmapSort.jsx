...
const SeasonSchoolHeatmapSort = ({ data: films }) => {
  ...
  const schoolObj = sortedSchools.map((school, i) => {
    return {
      name: school,
  ...
  const draw = () => {
    if (svgRef.current) {
      d3.select(svgRef.current)
        ...
        .attr(
          "fill",
          (d) => schoolObj.find((school) => d.school === school.name).color
        )
...
export default SeasonSchoolHeatmapSort;
